...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_URL || 'https://92.124.137.137:4000',
  timeout: 10000,
});
    ...
    if (token) {
      ...
      if (isTokenExpiringSoon(token)) {
        if (!isRefreshing) {
          isRefreshing = true;
          try {
            const response = await axios.post(`${import.meta.env.VITE_API_URL || 'https://92.124.137.137:4000'}/auth/refresh-token`, { token });
